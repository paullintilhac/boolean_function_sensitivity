...
class Trainer:
    ...
    # ---- NEW: build the reducer that implements subsampling (fixed-K or prob p)
    def _make_reducer(self, B: int, device):
        """
        Returns a function reduce(losses) that:
          - if subsample_k: picks K of B, returns mean(loss[idx]) * (B/K)  (unbiased)
          - elif subsample_p: Bernoulli mask with p; mean(loss[mask]) * (B/m) (unbiased)
          - else: mean(loss)
        The reducer (and its idx/mask) must be reused within both SAM passes.
        """
        if self.subsample_k is not None and 0 < self.subsample_k < B:
            K = self.subsample_k
            idx = torch.randperm(B, device=device)[:K]
            scale = B / K
            def reduce(losses):
                return losses[idx].mean() * scale
            return reduce

        if self.subsample_p is not None and 0.0 < self.subsample_p < 1.0:
            p = self.subsample_p
            ...
            if m == 0:
                ...
                def reduce(losses):
                    return losses[j].mean() * B
                return reduce
            scale = B / m
            def reduce(losses):
                return losses[mask].mean() * scale
            return reduce
    ...
    def _run_batch(self, inputs, targets):
        # ensure inputs live on this GPU (non-blocking H2D)
        inputs = inputs.to(self.gpu_id, non_blocking=True)
        # per-sample MSE
        def per_sample_losses(out, tgt):
            return (out.squeeze(-1) - tgt).pow(2)

        B = inputs.shape[0]
        reducer = self._make_reducer(B, device=self.gpu_id)

        # ---- SAM path ----
        if hasattr(self.optimizer, "base_optimizer"):
            with self.model.no_sync():
                out = self.model(inputs)
                losses = per_sample_losses(out, targets)
                loss = reducer(losses)
                loss.backward()
            self.optimizer.first_step(zero_grad=True)

            out = self.model(inputs)
            losses_perturbed = per_sample_losses(out, targets)
            loss_perturbed = reducer(losses_perturbed)
...
def parse_args():
    ...
    parser.add_argument('--subsample_k', type=int, default=None,
                        help='Use exactly K examples from each physical batch for the gradient (unbiased).')
    parser.add_argument('--subsample_p', type=float, default=None,
                        help='Bernoulli keep-prob p per example for the gradient (unbiased).')

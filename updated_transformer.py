...
class Transformer(torch.nn.Module):
    ...
    def __init__(self,dropout, N, hidden_dim, output_dim, num_heads, ff_dim, LNeps,rank,ln):
        ...
        # Data embedding
        self.embeddings = torch.nn.Embedding(2, hidden_dim).to(rank)
        if hidden_dim == 2:
    ...
    def forward(self, x):    
        ...
        # Get N+1 x N+1 positional embeddings
        pos = self.pos_embeddings(torch.arange(self.N+1).unsqueeze(0).repeat(batch_size, 1).to(self.rank))

        # Get N x hidden_dim positional embeddings
        dat = self.embeddings(inputNum).to(self.rank)

        # Add CLS Token
        dat = torch.cat([dat, torch.zeros((batch_size, 1, self.hidden_dim)).to(self.rank)], dim=1)

        # Concatenate positional and data embeddings

...
def load_train_objs(
    ...
    # ---------- MINIMAL FIX: avoid Python long overflow for large N ----------
    # We use torch.long (int64) to represent integers in [0, 2^N).
    # int64 max is ~9.22e18, so we must have 2^N - 1 <= 2^63 - 1.
    # Safely cap N so 2^N fits in int64. 2^62 ≈ 4.6e18 < 9.22e18.
    if N > 62:
        raise ValueError(
            f"N = {N} is too large for the current int64-based encoding "
            f"(2^N overflows torch.long). To handle N>62 we would need to "
            f"switch to an explicit bit-tensor representation instead of "
            f"packing assignments into a single integer."
        )
    # old version used Python random + torch.tensor([...]) and blew up for large N
...
def main(rank, args, world_size, coefs, combs, main_dir, deg, width, i, run_id):
    ...
    print("trainer.func_batch([2, 3]):", trainer.func_batch([2, 3]))

...
class Trainer:
    ...
    def _run_epoch(self,epoch):
        ...
        for idx, inputs in enumerate(self.train_data):
          ...
          # loss = self.model(inputs).sum()
          # work.wait()
          # #print("signal item: "  + str(signal.item()))
          # if signal.item() < self.world_size:
          #     break
          inputs.to(self.gpu_id)    
    ...
    def train(self,epochs: int):
        ...
        for epoch in range(epochs):
            ...
            if (((epoch % self.save_every)==0 or epoch_loss < 0.02) and self.gpu_id==0):
...
def main(rank, args,world_size,coefs,combs,main_dir,deg,width,i):
      ...
      print("finished training, cleaning up process group " + str(rank))
      #barrier()
    ...
    # for i in range(func_per_deg):
        ...
        for deg in range(1,6):
            ...
            #for width in [1]:
                ...
                mp.spawn(main,args=(arguments,arguments.world_size,coefs,combs,main_dir,deg,width,i,),nprocs=arguments.world_size,join=True,daemon=False)
